...
### Fixed
- 過去の「公的資料を根拠とする独自推計」という偏差値の説明が、関東の一部値に未検証の編集推計を含む実態を十分に示していなかったため訂正しました。方法・掲載基準・過去の倍率由来値を 2026-07-12 に非表示化した経緯を「偏差値の方法と限界」ページで公開し、根拠を確認できない学校は数値を掲載しない方針を明確化。利用規約 第 4 条・プライバシーポリシー 第 5 節の同種表現も「公的資料を参考にした編集推計」へ改定（各 v0.2）
